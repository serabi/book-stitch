...
class TestPairing:
    ...
    def test_pairing_success_stores_token_and_cards(self, client, mock_db):
        ...
        clone_response = make_response(200, cards_payload)

        with (
            patch.object(client.session, "post", return_value=chip_response),
            patch.object(client.session, "request", return_value=clone_response) as mock_request,
        ):
        ...
        # Clone call carried the code as JSON with bearer auth
        clone_call = mock_request.call_args
        assert clone_call.args[0] == "POST"
        assert clone_call.args[1].endswith("/chip/clone/code")
        assert clone_call.kwargs["json"] == {"code": "12345678"}
        assert clone_call.kwargs["headers"]["Authorization"] == "Bearer new-identity-token"

    def test_expired_code_reported(self, client):
        ...
        assert SETUP_CODE_URL in result["detail"]

    def test_revoked_token_reported_and_token_cleared(self, client):
        chip_response = make_response(200, {"identity": "tok"})
